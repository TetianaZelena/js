...
const getTotalFriendCount = users => {
    const total = users.flatMap(user => [...user.friends]);
    return total.length;
};
// Пиши код выше этой строки
...
const sortedByAscendingRating = [...books].sort((firstStudent, secondStudent) =>
  firstStudent.rating - secondStudent.rating);


const sortedByDescentingRating = [...books].sort((firstStudent, secondStudent) =>
  secondStudent.rating - firstStudent.rating);
...
const sortByDescendingFriendCount = users => {
     return [...users].sort((firstBalance, secondBalance) => secondBalance.friends.length-firstBalance.friends.length);
}; 
...
const getNamesSortedByFriendCount = users => {
  return [...users].sort((firstBalance, secondBalance) => firstBalance.friends.length-secondBalance.friends.length).map(user=>user.name);
