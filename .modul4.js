...
// const messages = [];
// for (position=0; position<orders.length; position+=1) { messages.push
// (composeMessage.call(orders[position],position));
// }
...
function composeMessage(position) {
 ...
 positions=+ this.position;
  composeMessage.call(position)
  }
return  `Готовим ${this.dish} для ${this.email}. Ваш заказ ${this.positions}-й в очереди.`
}

const messages = ['Готовим Burger для solomon@topmail.ua. Ваш заказ 1-й в очереди.', 'Готовим Pizza для artemis@coldmail.net. Ваш заказ 2-й в очереди.', 'Готовим Taco для jacob@mail.com. Ваш заказ 3-й в очереди.'];
